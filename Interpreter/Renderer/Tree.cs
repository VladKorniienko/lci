...
{
    class Tree : BuilderUtilities
    {
        private Tree child1;
        private Tree child2 = null;

        public Tree(string content) : base(content.Split('\n').Select(x => x.Length).Aggregate((x, y) => Math.Max(x, y)), content.Split('\n').Length)
        {
            this.InsertIntoPlane(content);
        }

        public Tree(string content, Tree child) : this(content)
        {
            this.child1 = child;

        }

        public Tree(string content, Tree child1, Tree child2) : this(content)
        {
            this.child1 = child1;
            this.child2 = child2;
        }

        public Tree GetChild1()
        {
            return this.child1;
        }

        public Tree GetChild2()
        {
            return this.child2;
        }
        ...
        private void InsertIntoPlane(string content)
        {
            ...
            this.Embedd(new BuilderUtilities(plane), PlaneAlignment.TOP_CENTER);
        }
        ...
        public override string ToString()
        {
            ...
            {
                BuilderUtilities child1Rep = new Tree(this.child1.ToString());
                BuilderUtilities child2Rep = new Tree(this.child2.ToString());
                ...
                child1Rep = new BuilderUtilities(childWidth, childHeight, child1Rep, PlaneAlignment.TOP_CENTER);
                child2Rep = new BuilderUtilities(childWidth, childHeight, child2Rep, PlaneAlignment.TOP_CENTER);
                ...
                var printPlane = new BuilderUtilities(thisRep);
            ...
            else if (child1 != null)
            {
                ...
                var printPlane = new BuilderUtilities(thisRep);
                printPlane.Embedd(this, PlaneAlignment.TOP_CENTER);
                printPlane.Embedd(new Tree(this.child1.ToString()), PlaneAlignment.BOTTOM_CENTER);
                return printPlane.ToString();
            }
            else
            {
