...
namespace LCI_Korniienko.Parser
{
    static class Parser
    {
        ...
        public static Expression Parse(string expr)
        {
            ...
            if (!"".Equals(expr))
            {
                ...
                else if (!new Regex("\\(|\\)|\\\\|\\.").IsMatch(expr))
                {
                    ...
                    Expression agg = ParseTerminal(parts[0]);
        ...
        private static Expression ParseTerminal(string expr)
        {
            ...
            {
                var temp = new Variable(expr);
                if (temp.CheckExpandableVariable())
                {
                    return temp;
                }
                else
                    return new Variable(expr[0].ToString());
            }
        }
    }
}
